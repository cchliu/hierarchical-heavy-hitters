...
class Pointer_Poisson(Pointer):
    def __init__(self, l, k, L, p0, eta, error, scale, logging_level, pid, xi):
        super(Pointer_Poisson, self).__init__(l, k, L, p0, eta, error, scale, logging_level, pid)
        self.xi = xi
    ...
    def _copyNode(self, node):
        ret = newNode_Poisson(node.l, node.k, node.x_mean, node.s)
    ...
    def _report(self):
        self.logger.info("Pntr %d: Report node (l, k) = (%d, %d) as the target " +
            ...
            "eta = %f, " +
            "error = %f, " +
            "xi = %f, leaf_alpha = %.12f", self.pid, self.curr_node.l, self.curr_node.k, \
            self.time_interval, self.curr_node.x_mean_net, \
            self.p0, self.eta, self.error, self.xi, self.leaf_alpha)
    ...
    def _test_func(self, node, alpha, beta, eta):
        x_mean, s = node.x_mean_net, node.s
        xi = self.xi
