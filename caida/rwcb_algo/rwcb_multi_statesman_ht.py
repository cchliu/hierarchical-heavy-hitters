"""Capsulate rw_cb algorithm into a class.
    ...
    Usage:
        ts = rwcb_algo(threshold, error, p_zero, b, u)
...
from rwcb_multi_hhh import subtract_hhh, update_hhh_set
from read_node import read_node
from tree_operation import parent, left_child, right_child 
...
def O_func_ht(x_mean, s, threshold, p1, p2, b, u, logger):
    X_mean = x_mean
    curr_s = float(s)
    # Calculate the equation
    part1 = 4*u**(1/float(b))*((math.log(2*curr_s**3/p1)/curr_s)**((b-1)/float(b)))
    equation_one = X_mean + part1
    logger.debug("X_mean: {0}, part1: {1}, threshold: {2}".format(X_mean, part1, threshold))
    if equation_one < threshold:
        logger.debug("Satisfy equation (31), return 1")
    ...
    part2 = 4*u**(1/float(b))*((math.log(2*curr_s**3/p2)/curr_s)**((b-1)/float(b)))
    equation_two = X_mean - part2
    logger.debug("X_mean: {0}, part2: {1}, threshold: {2}".format(X_mean, part2, threshold))
    if equation_two > threshold:
        logger.debug("Satisfy equation (32), return 2")
...
def cal_mean_ht(x_mean, s, node_val, p, b, u):
    bound = (u*(s+1)/float(math.log(1/p)))**(1/float(b))
    # Remove large samples
    add_val = 0
    if abs(node_val) <= bound:
        add_val = node_val

    new_mean = (x_mean*s + add_val)/float(s+1)
    return new_mean

class rwcb_algo(object):
    """Rewrite rwcb_multi algorithm as a state machine."""
    def __init__(self, threshold, p_zero, error, b, u):
        # Logger
    ...
    def state_one_leaf(self, line, leaf_level, HHH_nodes):
        ...
        self.ns.x_mean = cal_mean_ht(self.ns.x_mean, self.ns.s, node_val, self.p_zero, self.b, self.u)
        self.ns.s += 1.0
        ...
        O_func_outcome = O_func_ht(mod_x_mean, self.ns.s, self.threshold, self.p_zero, self.error, self.b, self.u, self.logger) 
    ...
    def state_one_nonleaf(self, line, leaf_level, HHH_nodes):
        ...
        if checking_level > self.root_level:
            ...
            O_func_outcome = O_func_ht(mod_x_mean, self.ns.s, self.threshold, self.p_zero, self.p_zero, self.b, self.u, self.logger)
            ...
            O_func_outcome = O_func_ht(mod_x_mean, self.ns.s, self.threshold, self.error, self.p_zero, self.b, self.u, self.logger)
        else:
    ...
    def state_two(self, line, leaf_level, HHH_nodes):
        ...
        self.ns_reading.x_mean = cal_mean_ht(self.ns_reading.x_mean, self.ns_reading.s, node_val, self.p_zero, self.b, self.u)
        ...
        O_func_outcome = O_func_ht(mod_x_mean, self.ns_reading.s, self.threshold, self.p_zero, self.p_zero, self.b, self.u, self.logger)
        if O_func_outcome == 0:
            return "state_two" 
        elif O_func_outcome == 2:
